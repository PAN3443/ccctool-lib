...
class CMS {
  ...
  calculateBandColor(_bandID, _val, _space) {
    ...
    switch (true) {
      ...
      default:
        ...
        if (color1.isUndefined()) {
          //////////////////////////////////////////
          /////////      Constant Band     /////////
          //////////////////////////////////////////
          return color2.getColorJSON(space[1]);
        } else {
          //////////////////////////////////////////
          /////////       Scaled Band      /////////
          //////////////////////////////////////////
          if ((this.interpolationSpace == "de94-ds" || this.interpolationSpace == "de2000-ds") && this.p_deltaColors[_bandID].length > 0) {
            /////////////////////////////////////////
            ...
            switch (index) {
              case 0:
                deltaColor_V1 = colorToVector(color1.getColorJSON("lab"));
                deltaColor_V2 = colorToVector(this.p_deltaColors[_bandID][index][0]);
                ratio = getRatio(this.p_keys[_bandID].getRef(), this.p_deltaColors[_bandID][index][1], _val);
                break;
              case 0:
                deltaColor_V1 = colorToVector(this.p_deltaColors[_bandID][index][0]);
                deltaColor_V2 = colorToVector(color2.getColorJSON("lab"));
                ratio = getRatio(this.p_deltaColors[_bandID][index - 1][1], this.p_deltaColors[_bandID][index][1], _val);
                break;
              default:
                deltaColor_V1 = colorToVector(this.p_deltaColors[_bandID][index][0], "lab");
                deltaColor_V2 = colorToVector(this.p_deltaColors[_bandID][index][0], "lab");
            ...
            return updateColorToSpace(vectorToColor(linearInterpolation(deltaColor_V1, deltaColor_V2, ratio), "lab"), space[1]);
          } else {
            switch (this.p_interpolationType) {
              case "linear":
                ...
                /////////////////////////////////////////////////////////////////
                let color_V1 = colorToVector(color1.getColorJSON(this.p_interpolationSpace));
                let color_V2 = colorToVector(color2.getColorJSON(this.p_interpolationSpace));
                ratio = getRatio(this.p_keys[_bandID].getRef(), this.p_keys[_bandID + 1].getRef(), _val);
                return updateColorToSpace(vectorToColor(linearInterpolation(color_V1, color_V2, ratio), this.p_interpolationSpace), this.p_interpolationSpace);
              case "spline":
                /////////////////////////////////////////////////////////////////
                ...
                if (refColors == undefined) {
                  let color_V1 = colorToVector(color1.getColorJSON(this.p_interpolationSpace));
                  let color_V2 = colorToVector(color2.getColorJSON(this.p_interpolationSpace));
                  return updateColorToSpace(vectorToColor(linearInterpolation(color_V1, color_V2, ratio), this.p_interpolationSpace), space[1]); // do linear if spline is not possible
                } else {
                  return updateColorToSpace(vectorToColor(splineInterpolation(refColors[0], refColors[1], refColors[2], refColors[3], ratio, 1.0), this.p_interpolationSpace), space[1]);
                }
            }
          }
        }
  ...
  _getSplineColors(_bandID) {
    ...
    c1 = colorToVector(this.getKeyCR_JSON(_bandID, this.interpolationSpace));

    c2 = colorToVector(this.getKeyCL_JSON(_bandID + 1, this.interpolationSpace));

    if (!existingC1) {
      c0 = [0, 0, 0]; // every value is zero and has no influence
    } else c0 = colorToVector(this.getKeyCR_JSON(_bandID - 1, this.interpolationSpace));

    if (!existingC3) {
      c3 = [0, 0, 0]; // every value is zero and has no influence
    } else {
      c3 = colorToVector(this.getKeyCL_JSON(_bandID + 2, this.interpolationSpace));
    }
  ...
  deleteBand(index) {
    let tmpRightColor = this.getKeyCR_JSON(index + 1, "lab");
