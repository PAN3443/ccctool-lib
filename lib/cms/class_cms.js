...
class CMS {
  ...
  #probeSetArray = [];
  /// work
  #workOn = false; // let some functions know that another method work on the CMS and not all guardian clauses are nessesarry or rather obstructive
  ...
  calculateBandColor(_bandID, _val, _space) {
    ...
    switch (true) {
      case this.#keys[_bandID].getRef() == _val:
        ...
        /////////   Hit the First Key    /////////
        //////////////////////////////////////////
        return this._getBandKeyColor(_bandID, true);
      case this.#keys[_bandID + 1].getRef() == _val:
        //////////////////////////////////////////
        /////////   Hit the Second Key    /////////
        //////////////////////////////////////////
        // check middle of triple color
        return this._getBandKeyColor(_bandID, false);
      default:
        // value is between index-1 and index
        // we need to work with the color in the colorspace, which we use for interpolation
        let color1 = this.getKeyCR(_bandID, this.interpolationSpace);
        let color2 = this.getKeyCL(_bandID + 1, this.interpolationSpace);
  ...
  _getBandKeyColor(_bandID, _isStartKey, _space) {
    ...
    switch (this.getKeyType(keyIndex)) {
      ...
      case "left":
        ...
        return this.#keys[keyIndex].getMoT() ? this.getKeyCL(keyIndex + 1, _space) : this.getKeyCL(keyIndex, _space);
      case "twin":
        return this.#keys[keyIndex].getMoT() ? this.getKeyCR(keyIndex, _space) : this.getKeyCL(keyIndex, _space);
      //case "right":
  ...
  calcReverse() {
    ...
    for (let i = 0; i < this.getKeyLength(); i++) {
      if ((this.getKeyType(i) === "nil key" || this.getKeyType(i) === "left key") && i != this.getKeyLength() - 1) {
        this.setKeyCR(i, this.getKeyCL(i + 1, this.#interpolationSpace));
        this.setKeyCL(i + 1, undefined);
      }

      let newPos = startPos + (endPos - this.getKeyRef(i));
      let tmpColor = this.getKeyCL(i, this.#interpolationSpace);
      this.setKeyCL(i, this.getKeyCR(i, this.#interpolationSpace));
  ...
  getCMSJSON() {
    ...
    for (let i = 0; i < this.getKeyLength(); i++) {
      cmsJSON.keys.push(this.#keys[i].getKeyJSON());
    }
  ...
  setByJSON(_cmsJSON) {
    ...
    if (isNumber(_cmsJSON.jnd)) this.#jnd = _cmsJSON.jnd;
  }
    ...
    switch (_index) {
      ...
      default:
        ...
        for (let i = _cmsJSON.keys.length - 2; i >= 0; i--) {
          ...
          this._insertKey(_index, new class_Key(_cmsJSON.keys[i].cL, _cmsJSON.keys[i].cR, endPos, _cmsJSON.keys[i].isBur, _cmsJSON.keys[i].mot));
        }
  ...
  getKeyClone(_index) {
    ...
    return _keyJsonToInstance(this.#keys[_index].getKeyJSON());
  }
        ...
        /*if (this.interpolationType === "spline") {
          if (_index - 2 >= 0) {
            if (this.#keys[_index - 1].getKeyType() === "dual key") {
              ...
              if (_index - 3 >= 0) {
                if (this.#keys[_index - 2].getKeyType() === "dual key") this._updateBandDeltaColors(index - 3);
              }
            }
          }
        }*/
          ...
          if (index + 1 < this.getKeyLength()) {
            if (this.#keys[index + 1].getKeyType() === "dual key") {
              ...
              if (index + 2 < this.getKeyLength()) {
                if (this.#keys[index + 2].getKeyType() === "dual key") this._updateBandDeltaColors(index + 2);
              }
            }
          }
        }*/
  ...
  getKeyCL(_index, _space) {
    ...
    if (this.#keys[_index].getCL() !== undefined) return updateColorToSpace(this.#keys[_index].getCL(), _space);
    return undefined;
  }
  ...
  getKeyCR(_index, _space) {
    ...
    if (this.#keys[_index].getCR() !== undefined) return updateColorToSpace(this.#keys[_index].getCR(), _space);
    return undefined;
  }
  ...
  _pushKey(_key) {
    ...
    this.#keys.push(_key);
    if (this.getKeyLength() > 1) this.#deltaColors.push([]);
  }
...
function _keyJsonToInstance(_key) {
  let tmpKey = new KeyCMS(undefined, undefined, 0);
