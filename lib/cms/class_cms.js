...
const { isJSON, isNumber, checkIndex, checkInterpolationType } = require("../helper/guardClauses.js");
...
class CMS {
  ...
  calcReverse() {
    ...
    for (let i = 0; i < this.#keyArray.length; i++) {
      //
      if ((this.getKeyType(i) === "nil key" || this.getKeyType(i) === "left key") && i != this.#keyArray.length - 1) {
        this.setRightKeyColor(i, this.getLeftKeyColor(i + 1, "lab"));
        this.setLeftKeyColor(i + 1, undefined);
      }

      let newPos = startPos + (endPos - this.getKeyRef(i));

      let tmpColor = this.getLeftKeyColor(i, "lab");
      this.setLeftKeyColor(i, this.getRightKeyColor(i, "lab"));
      this.setRightKeyColor(i, tmpColor);
  ...
  setCMSJSON(_cmsJSON) {
    if (!isJSON(_cmsJSON)) throw new TypeError('Error (CMS) :: Function "setCMSJSON" :: Incorrect parameter! The parameter "_cmsJSON" is not a JSON!');
    if(!_cmsJSON.hasOwnProperty("keyArray")) throw new TypeError('Error (CMS) :: Function "setCMSJSON" :: Incorrect parameter! The parameter "_cmsJSON" has not the property "keyArray"!');
    else(){
      
    }


    if(_cmsJSON.hasOwnProperty("interpolationSpace"))
      this.#interpolationSpace = cmsJSON.interpolationSpace
    else
      console.warn('Warning (CMS) :: Function "setCMSJSON" :: Incorrect parameter! The parameter "_cmsJSON" is not a JSON!);
   ...
   *************************************************************************************************/
  deleteKey(_index) {
    this.#keyArray.splice(_index, 1);
    this.#deltaColors.splice(_index, 1);

    if (_index != 0 && _index != this.keyArray.length - 1) this.updateBandDeltaColors(_index - 1);
  }
      ...
      if (this.interpolationType === "spline") {
        if (index - 2 >= 0) {
          if (this.keyArray[index - 1].getKeyType() === "dual key") {
            this.updateBandDeltaColors(index - 2);
            if (index - 3 >= 0) {
              if (this.keyArray[index - 2].getKeyType() === "dual key") this.updateBandDeltaColors(index - 3);
            }
          }
        }
      }
  ...
  setRightKeyColor(index, color) {
    ...
    if (index != this.keyArray.length - 1) {
      this.updateBandDeltaColors(index);
      if (this.interpolationType === "spline") {
        if (index + 1 < this.keyArray.length) {
          if (this.keyArray[index + 1].getKeyType() === "dual key") {
            this.updateBandDeltaColors(index + 1);
            if (index + 2 < this.keyArray.length) {
              if (this.keyArray[index + 2].getKeyType() === "dual key") this.updateBandDeltaColors(index + 2);
            }
          }
        }
      }
