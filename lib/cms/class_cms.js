...
const { isCMSJSON, isNumber, checkIndex, checkInterpolationType, isColorJSON, isCMSKey } = require("../helper/guardClauses.js");
...
class CMS {
  ...
  #colorBelow = { space: "rgb", c1: 0, c2: 0, c3: 0 };
  #colorAbove = { space: "rgb", c1: 0, c2: 0, c3: 0 };
  #keys = [];
  #deltaColors = [];
  ...
  calculateColor(_val, _space) {
    ...
    if (_val > this.getKeyRef(this.#keys.length - 1)) {
      return this.getAboveColor(space[1]);
    }

    let index = keys.findIndex(function (key) {
  ...
  calculateBandColor(_bandID, _val, _space) {
    ...
    if (!checkIndex(_bandID, this.#keys.length - 1)) throw new TypeError('Error (CMS) :: Function "calculateBandColor" :: Incorrect bandID! The value ' + _bandID + " is not a valid index!");

    switch (true) {
      case this.#keys[_bandID].getRef() == _val:
        //////////////////////////////////////////
        /////////   Hit the First Key    /////////
        //////////////////////////////////////////
        return getBandKeyColor(_bandID, true);
      case this.#keys[_bandID + 1].getRef() == _val:
        //////////////////////////////////////////
        /////////   Hit the Second Key    /////////
        //////////////////////////////////////////
          ...
          if (this.#deltaColors[_bandID].length > 0) {
            ...
            switch (index) {
              case 0:
                ...
                ratio = getRatio(this.#keys[_bandID].getRef(), this.#deltaColors[_bandID][index][1], _val);
                break;
              case 0:
              ...
              default:
                deltaColor_V1 = colorToVector(updateColorToSpace(this.#deltaColors[_bandID][index][0], this.interpolationSpace));
                deltaColor_V2 = colorToVector(updateColorToSpace(this.#deltaColors[_bandID][index][0], this.interpolationSpace));
                ratio = getRatio(this.#deltaColors[_bandID][index][1], this.#keys[_bandID + 1].getRef(), _val);
                break;
            }
            ...
            switch (key) {
              case "linear":
                ...
                let ratio = getRatio(this.#keys[_bandID].getRef(), this.#keys[_bandID + 1].getRef(), _val);
  ...
  static getSplineColors(_bandID) {
    ...
    if (this.getKeyType(_bandID + 1) == "left" || this.getKeyType(_bandID + 1) == "twin" || _bandID + 1 == this.keys.length - 1) {
      // this.keys.length-1 is alwas a left key. For the push creation this don't have to be.
      existingC3 = false;
    }
  ...
  static getBandKeyColor(_bandID, _isStartKey, _space) {
    ...
    switch (this.keys[keyIndex].getKeyType()) {
      case "nil":
        return this.getLeftKeyColor(keyIndex + 1, _space);
      case "left":
        // if not static, we would have to check if the
        return this.#keys[keyIndex].getMoT() ? this.getLeftKeyColor(keyIndex + 1, _space) : this.getLeftKeyColor(keyIndex, _space);
      case "twin":
        return this.#keys[keyIndex].getMoT() ? this.getRightKeyColor(keyIndex, _space) : this.getLeftKeyColor(keyIndex, _space);
      //case "right":
  ...
  searchForContinuousSections(_startKey, _endKey) {
    if (!checkIndex(_startKey, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "calculateBandColor" :: Incorrect _startKey! The value ' + _startKey + " is not a valid index!");
    if (!checkIndex(_endKey, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "calculateBandColor" :: Incorrect bandID! The value ' + _endKey + " is not a valid index!");
  ...
  calcReverse() {
    ...
    let endPos = this.getKeyRef(this.#keys.length - 1);

    for (let i = 0; i < this.#keys.length; i++) {
      //
      if ((this.getKeyType(i) === "nil key" || this.getKeyType(i) === "left key") && i != this.#keys.length - 1) {
        this.setKeyCR(i, this.getLeftKeyColor(i + 1, "lab"));
      ...
      this.setKeyCL(i, this.getRightKeyColor(i, "lab"));
  ...
  setAutoRange(_newStart, _newEnd) {
    ...
    let currentdistance = this.getKeyRef(this.#keys.length - 1) - currentStart;
  ...
  equalKeyIntervals() {
    if (this.#keys.length > 2) {
      let equalDis = Math.abs(this.getKeyRef(this.#keys.length - 1) - this.getKeyRef(0)) / (this.#keys.length - 1);
  ...
  getCMSJSON() {
    let cmsJSON = {};
    cmsJSON.name = this.name;
    cmsJSON.interpolationSpace = this.#interpolationSpace;
    cmsJSON.interpolationType = this.#interpolationType; // linear or spline or optimization
    cmsJSON.colorNaN = this.#colorNaN;
    cmsJSON.colorBelow = this.#colorBelow;
    cmsJSON.colorAbove = this.#colorAbove;
    cmsJSON.keys = [];

    cmsJSON.jnd = this.#jnd;
  ...
  setByJSON(_cmsJSON) {
    ...
    this.clear();
    this.name = _cmsJSON.name;

    if (typeof _cmsJSON.interpolationType === "string") if (_cmsJSON.interpolationType === "linear" || _cmsJSON.interpolationType === "spline") this.interpolationType = _cmsJSON.interpolationType;

    let space = checkColorSpaceNotation(_cmsJSON.interpolationSpace);
    if (space[0]) this.interpolationSpace = space[1];

    if (isColorJSON(_cmsJSON.colorNaN)) this.colorNaN = _cmsJSON.colorNaN;
    if (isColorJSON(_cmsJSON.colorBelow)) this.colorBelow = _cmsJSON.colorBelow;
    if (isColorJSON(_cmsJSON.colorAbove)) this.colorAbove = _cmsJSON.colorAbove;

    /// Keys ///
    for (let index = 0; index < _cmsJSON.keys.length; index++) {
      this.pushKey(_cmsJSON.keys);
    }
  }
  ...
  insertCMS(_cmsJSON, _index) {
    ...
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getKey" :: Incorrect index!');
    ...
    switch (_index) {
      case this.getKeyLength() - 1:
        ...
        let dist = Math.abs(tmpVal - this.getKeyRef(_index - 1)) * 0.5;
        ...
        for (let i = 1; i < _cmsJSON.keys.length; i++) {
          ...
          if (i == _cmsJSON.keys.length - 1) this.pushKey(new KeyColor(_cmsJSON.keys[i].cL, _cmsJSON.keys[i].cR, tmpVal, _cmsJSON.keys[i].isBur, _cmsJSON.keys[i].mot));
          else this.pushKey(new KeyColor(_cmsJSON.keys[i].cL, _cmsJSON.keys[i].cR, startPos, _cmsJSON.keys[i].isBur, _cmsJSON.keys[i].mot));
        }
        break;

      default:
        let startPos = this.getKeyRef(_index);
        let dist = Math.abs(this.getKeyRef(_index + 1) - startPos) * 0.5;
        let endPos = startPos + dist;

        this.setKeyRef(_index, endPos);
        let oldColor = this.getLeftKeyColor(_index, "lab");
        ...
        for (let i = _cmsJSON.keys.length - 2; i >= 0; i--) {
          ...
          this.insertKey(_index, new class_Key(_cmsJSON.keys[i].cL, _cmsJSON.keys[i].cR, endPos, _cmsJSON.keys[i].isBur, _cmsJSON.keys[i].mot));
        }
  ...
  static updateBandDeltaColors(_bandID) {}
   ...
   *************************************************************************************************/
  deleteKey(_index) {
    if (!checkIndex(_index, this.#keys.length)) return;
    this.#keys.splice(_index, 1);
    this.#deltaColors.splice(_index, 1);
    if (_index != 0 && _index != this.keys.length - 1) this.updateBandDeltaColors(_index - 1);
  }
  ...
  getKeyClone(_index) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getKeyClone" :: Incorrect index!');
    return keyJsonToInstance(this.#keys[index].getKeyJSON());
  }

  setKeyCL(_index, cL) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "setKeyCL" :: Incorrect index!');
    this.#keys[_index].setCL(cL);
    if (_index != 0) {
      this.updateBandDeltaColors(_index - 1);
      /*if (this.interpolationType === "spline") {
        if (_index - 2 >= 0) {
          if (this.keys[_index - 1].getKeyType() === "dual key") {
            this.updateBandDeltaColors(_index - 2);
            if (_index - 3 >= 0) {
              if (this.keys[_index - 2].getKeyType() === "dual key") this.updateBandDeltaColors(index - 3);
            }
          }
        }
      }*/
  ...
  setKeyCR(_index, cR) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "setKeyCL" :: Incorrect index!');
    this.keys[_index].setCR(cR);

    if (_index != this.#keys.length - 1) {
      this.updateBandDeltaColors(_index);
      /*if (this.interpolationType === "spline") {
        if (index + 1 < this.keys.length) {
          if (this.keys[index + 1].getKeyType() === "dual key") {
            this.updateBandDeltaColors(index + 1);
            if (index + 2 < this.keys.length) {
              if (this.keys[index + 2].getKeyType() === "dual key") this.updateBandDeltaColors(index + 2);
            }
          }
        }
      }*/
  ...
  getLeftKeyColor(_index, _space) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getLeftKeyColor" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return updateColorToSpace(this.#keys[_index].getCL(), _space);
  }

  getRightKeyColor(_index, _space) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getRightKeyColor" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return updateColorToSpace(this.#keys[_index].getCR(), _space);
  }

  getKeyType(_index) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getKeyType" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return this.keys[_index].getType();
  }

  getMoT(_index) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getMoT" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return this.keys[_index].getMoT();
  }

  setMoT(_index, _mot) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "setMoT" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    this.keys[_index].setMoT(_mot);
  }

  insertKey(_index, _key) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "insertKey" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    if (isCMSKey(_key)) _key = keyJsonToInstance(_key);
    ...
    this.#keys.splice(_index, 0, _key);
  ...
  pushKey(_key) {
    ...
    if (this.keys.length > 1) this.#deltaColors.push([]);
  }

  getBur(_index) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getBur" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return this.#keys[_index].getBur();
  }

  setBur(_index, _isBur) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "setBur" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    this.keys[_index].setBur(_isBur);
  }

  deleteBand(index) {
    let tmpRightColor = this.getRightKeyColor(index + 1, "lab");
    this.keys[index].setKeyCR(tmpRightColor);
  ...
  getRefRange() {
    if (this.#keys.length > 1) {
      return this.#keys[this.#keys.length - 1].getRef() - this.#keys[0].getRef();
    } else {
      return 0;
    }
  }
  ...
  getKeyRef(_index) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "getKeyRef" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    if (_index < 0 || _index >= this.#keys.length) return undefined;

    return this.#keys[_index].getRef();
  }
  ...
  setKeyRef(_index, _ref) {
    if (!checkIndex(_index, this.#keys.length)) throw new TypeError('Error (CMS) :: Function "setKeyRef" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    this.#keys[_index].setRef(_ref);
...
function keyJsonToInstance(_key) {
  let tmpKey = new KeyColor(undefined, undefined, 0);
