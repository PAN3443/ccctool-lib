...
class CMS {
  ...
  calcReverse() {
    ...
    for (let i = 0; i < this.getKeyLength(); i++) {
      ...
      if ((this.getKeyType(i) === "nil key" || this.getKeyType(i) === "left key") && i != this.getKeyLength() - 1) {
        this.setKeyCR(i, this.getKeyCL(i + 1, "lab"));
        this.setKeyCL(i + 1, undefined);
      }

      let newPos = startPos + (endPos - this.getKeyRef(i));

      let tmpColor = this.getKeyCL(i, "lab");
      this.setKeyCL(i, this.getKeyCR(i, "lab"));
  ...
  setByJSON(_cmsJSON) {
    ...
    for (let index = 0; index < _cmsJSON.keys.length; index++) {
      this._pushKey(keyJsonToInstance(_cmsJSON.keys[index]));
    }
  ...
  getKeyClone(_index) {
    ...
    return keyJsonToInstance(this.#keys[_index].getKeyJSON());
  }
