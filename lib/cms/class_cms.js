...
class CMS {
          ...
          if (this.#deltaColors[_bandID].length > 0) {
            ...
            switch (key) {
              case "linear":
                ...
                ratio = getRatio(this.#keys[_bandID].getRef(), this.#keys[_bandID + 1].getRef(), _val);
  ...
  calcReverse() {
    ...
    for (let i = 0; i < this.getKeyLength(); i++) {
      //
      if ((this.getKeyType(i) === "nil key" || this.getKeyType(i) === "left key") && i != this.getKeyLength() - 1) {
  ...
  getLeftKeyColor(_index, _space) {
    if (!checkIndex(_index, this.getKeyLength())) throw new TypeError('Error (CMS) :: Function "getLeftKeyColor" :: Incorrect _index! The value ' + _index + " is not a valid index!");
    return updateColorToSpace(this.#keys[_index].getCL(), _space);
  }

  getRightKeyColor(_index, _space) {
    if (!checkIndex(_index, this.getKeyLength())) throw new TypeError('Error (CMS) :: Function "getRightKeyColor" :: Incorrect _index! The value ' + _index + " is not a valid index!");
  ...
  insertKey(_index, _key) {
    if (!checkIndex(_index, this.getKeyLength())) throw new TypeError('Error (CMS) :: Function "insertKey" :: Incorrect _index! The value ' + _index + " is not a valid index!");
  ...
  addKey(_key) {
    ...
    switch (this.getKeyLength()) {
      case 0:
        ...
        break;
      case 1:
        if (_key.ref == this.#keys[0].getRef()) throw new TypeError('Error (CMS) :: Function "addKey" :: The reference value of added key is equal to the reference value of an existing key!');
        break;
      default:
        // find position
        let index = undefined;
  ...
  static pushKey(_key) {
    ...
    /*if (this.getKeyLength() != 0) {
      if (_key.getRef() <= this.getKeyRef(this.getKeyLength() - 1)) throw new TypeError('Error (CMS) :: Function "pushKey" :: Incorrect parameter! The reference value of pushed key need to be larger than the reference value of the previouse key!');
    }*/
    this.#keys.push(_key);
  ...
  getRefRange() {
    if (this.getKeyLength() > 1) {
      return this.#keys[this.getKeyLength() - 1].getRef() - this.#keys[0].getRef();
    } else {
      return 0;
    }
  }
