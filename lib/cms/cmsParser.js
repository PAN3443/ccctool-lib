...
module.exports = {
        ...
        for (let i = 0; i < jsonObj[0][pointName].length / 4; i++) {
          ...
          switch (i) {
            case 0:
              ...
              let tmpColor2 = { space: cSpace, c1: val1_Next, c2: val2_Next, c3: val3_Next };

              if (equalColorInfo(tmpColor, tmpColor2)) {
                // nil key
                parsedCMS.addKey(new KeyCMS(undefined, undefined, x, true, false));
              } else {
                // right key
                parsedCMS.addKey(new KeyCMS(undefined, tmpColor, x, true, false));
              }
              break;
            case lastIndex:
              // right key
              parsedCMS.addKey(new KeyCMS(tmpColor, undefined, x, true, false));
              break;
            default:
              if (hasKeyInfo) {
              ...
              if (x_Previous == x) {
                ...
                if (x != x_Next) {
                  // dual key
                  parsedCMS.addKey(new KeyCMS(tmpColor, tmpColor, x, false, false));
                }
              }
          } //switch
        }
        parsedCMS.setWorkOn(false);
  ...
  cmsParser_XML: function (xmlString) {
    ...
    if (colormapObject.length > 0) {
      ...
      if (colormapObject[0].hasAttribute("name")) parsedCMS.setColormapName(colormapObject[0].getAttribute("name"));
  ...
  cmsParser_CSV: function (_csvString) {
    ...
    let space = checkCSVColorspace(csvlines[0]);
    let val1_RatioFactor = 1;
    let val2_RatioFactor = 1;
    let val3_RatioFactor = 1;
    ...
    for (let i = 1; i < csvlines.length; i++) {
      ...
      switch (i) {
        case 1:
          let keyData_Next = csvlines[i + 1].split(/[;,]+/);
