...
//////////////      HEADER      //////////////
//////////////////////////////////////////////
// File :: Unit-Tests for Class CMS Key
// Author :: Pascal Nardini
// License :: MIT
// https://jestjs.io/docs/en/expect

const { Key } = require("../../../cms/class_cmsKey.js");
...
test("Color Class :: Constructor :: Correct Input", () => {
  const key = new Key({ space: "rgb", c1: 0.3, c2: 0.3, c3: 0.3 }, { space: "rgb", c1: 0.7, c2: 0.7, c3: 0.7 }, 0.3, true, true);
...
test("Color Class :: setCL :: Correct Input", () => {
  const key = new Key(undefined, undefined, 0, false, false);
...
test("Color Class :: determineType() & getType() :: Input (Right Key)", () => {
  const key = new Key(undefined, { space: "rgb", c1: 0.5, c2: 0.5, c3: 0.5 }, 0, false, false);
  expect(key.getType()).toBe("right");
});

test("Color Class :: determineType() & getType() :: Input (Left Key)", () => {
  const key = new Key({ space: "rgb", c1: 0.5, c2: 0.5, c3: 0.5 }, undefined, 0, false, false);
  expect(key.getType()).toBe("left");
});

test("Color Class :: Constructor :: Correct Input", () => {
  const key = new Key({ space: "rgb", c1: 0.5, c2: 0.5, c3: 0.5 }, { space: "rgb", c1: 0.5, c2: 0.5, c3: 0.5 }, 0, false, false);
  expect(key.getType()).toBe("dual");
});

test("Color Class :: Constructor :: Correct Input", () => {
  const key = new Key({ space: "rgb", c1: 0.7, c2: 0.7, c3: 0.7 }, { space: "rgb", c1: 0.3, c2: 0.3, c3: 0.3 }, 0, false, false);
