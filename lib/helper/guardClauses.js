...
exports.isCMSKey = (_keyJSON) => {
  ...
  if (!(this.isJSONEmpty(_keyJSON.cL) || _keyJSON.cL === undefined || this.isColorJSON(_keyJSON.cL))) return false;
  if (!(this.isJSONEmpty(_keyJSON.cR) || _keyJSON.cR === undefined || this.isColorJSON(_keyJSON.cR))) return false;

  return true;
};

exports.isJSONEmpty = (_JSON) => {
  if (!this.isJSON(_JSON)) return false;
...
exports.isCMSJSON = (_cmsJSON) => {
  ...
  for (let i = 0; i < _cmsJSON.keys.length; i++) {
    ...
    switch (i) {
      case 0:
        if (this.isColorJSON(_cmsJSON.keys[i].cL)) {
        ...
        } else if (_cmsJSON.keys[i].cL != undefined && !this.isJSONEmpty(_cmsJSON.keys[i].cL)) return false; // start key is nil or right key, both have a undefined left key color
        break;
      case lastIndex:
        if (this.isColorJSON(_cmsJSON.keys[i].cR)) {
        ...
        } else if (_cmsJSON.keys[i].cR != undefined && !this.isJSONEmpty(_cmsJSON.keys[i].cR)) return false; // end key is left key (=> undefined right key color)
        if (this.isColorJSON(_cmsJSON.keys[i].cL)) {
          if (_cmsJSON.keys[i].cL.space === undefined || _cmsJSON.keys[i].cL.c1 === undefined || _cmsJSON.keys[i].cL.c2 === undefined || _cmsJSON.keys[i].cL.c3 === undefined) return false;
        } else if (_cmsJSON.keys[i].cL === undefined || this.isJSONEmpty(_cmsJSON.keys[i].cL)) return false; // end key is left key (=> defined left key color)
        break;
      default:
        if (this.isColorJSON(_cmsJSON.keys[i].cL)) {
          if (_cmsJSON.keys[i].cL.space === undefined || _cmsJSON.keys[i].cL.c1 === undefined || _cmsJSON.keys[i].cL.c2 === undefined || _cmsJSON.keys[i].cL.c3 === undefined) return false;
        } else if (_cmsJSON.keys[i].cL === undefined || this.isJSONEmpty(_cmsJSON.keys[i].cL)) return false; // only the start keys are allowed to have a undefined left color.
        break;
    }
  }
