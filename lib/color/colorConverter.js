import { isNumber, isMathMatrix, isColorJSON } from "../helper/guardClauses.js";
import { mXv, invert3x3, atan2_360Degree, degree360ToRad } from "../helper/math.js";

import { cccToolProperties } from "../properties.js";

export const hsv2rgb = (_colorJSON) => {
...
export const rgb2hsv = (_colorJSON) => {
...
export const rgb2xyz = (_colorJSON, _tm) => {
...
export const xyz2rgb = (_colorJSON, _tm) => {
  ...
  else var_rgb[0] = 12.92 * var_rgb[0];
...
export const xyz2lms = (_colorJSON, _tm) => {
...
export const lms2xyz = (_colorJSON, _tm) => {
...
export const lms2lms_CB = (_colorJSON, _tm) => {
...
export const xyz2lab = (_colorJSON, _REFX, _REFY, _REFZ) => {
  if (!isNumber(_REFX)) _REFX = cccToolProperties.REFX; // if undefined use default
  if (!isNumber(_REFY)) _REFY = cccToolProperties.REFY; // if undefined use default
  if (!isNumber(_REFZ)) _REFZ = cccToolProperties.REFZ; // if undefined use default
...
export const lab2xyz = (_colorJSON, _REFX, _REFY, _REFZ) => {
  if (!isNumber(_REFX)) _REFX = cccToolProperties.REFX; // if undefined use default
  if (!isNumber(_REFY)) _REFY = cccToolProperties.REFY; // if undefined use default
  if (!isNumber(_REFZ)) _REFZ = cccToolProperties.REFZ; // if undefined use default
...
export const lab2lch = (_colorJSON) => {
...
export const lch2lab = (_colorJSON) => {
...
export const lab2din99 = (_colorJSON, _DIN99KE, _DIN99KCH) => {
  if (!isNumber(_DIN99KE)) _DIN99KE = cccToolProperties.DIN99KE; // if undefined use default
  if (!isNumber(_DIN99KCH)) _DIN99KCH = cccToolProperties.DIN99KCH; // if undefined use default
...
export const din992lab = (_colorJSON, _DIN99KE, _DIN99KCH) => {
  if (!isNumber(_DIN99KE)) _DIN99KE = cccToolProperties.DIN99KE; // if undefined use default
  if (!isNumber(_DIN99KCH)) _DIN99KCH = cccToolProperties.DIN99KCH; // if undefined use default
...
function getTransferMatrix(_fctName, _tmName) {
  ...
  if (!cccToolProperties.hasOwnProperty(_tmName)) _tmName = "use_default"; // check if transfer matrix exist; if not => use default
  ...
  return cccToolProperties[_tmName];
}
...
function falseTM(_type) {
  ...
  return cccToolProperties[_type + "::Default"]; // the default properties only include the key name of the default _tm. So this return the real TM name and not e.g, "TMRGB2XYZ::Default"
}
